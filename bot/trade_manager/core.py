...
class TradeManager:
    ...
    async def calculate_position_size(
        ...
        try:
            ...
            if stop_loss_distance <= 0:
                ...
                return 0.0
            max_position_value = equity * self.leverage * self.max_position_pct
    ...
    def calculate_stop_loss_take_profit(
        ...
        return order_utils.calculate_stop_prices(side, price, atr, sl_multiplier, tp_multiplier)
    ...
    async def open_position(self, symbol: str, side: str, price: float, params: Dict):
        try:
            ...
            order = await order_utils.retry_async(
                ...
                delay=retry_delay,
                label=f"order for {symbol}",
                loggers=(logger,),
                success_checker=order_utils.order_retcode_success,
            )
            if order is None:
                logger.error(
                ...
                return
            if isinstance(order, dict) and not order_utils.order_has_identifier(order):
                logger.error(
