...
This module intentionally lives at the repository root so it can be imported as
``bot.utils`` thanks to the path mangling performed in :mod:`bot.__init__`.  A
recent refactor accidentally removed several fundamental imports which meant
the module failed to initialise before key helpers were defined.  Restoring the
imports keeps :mod:`bot.utils` self-contained and prevents confusing import
errors reported by the CI suite.
"""
