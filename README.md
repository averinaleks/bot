...
1. Установите зависимости. Варианты для GPU и CPU:
   ...
   её перед `pytest`, чтобы все проверки прошли успешно.
    ...
    Если у вас другая версия CUDA, установите подходящий пакет CuPy вручную,
    например `pip install cupy-cuda11x`. Если CuPy отсутствует, бот
    автоматически переключается на CPU‑режим. Список `requirements-cpu.txt`
