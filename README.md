...
Если `rl_use_imitation` выставлен в `true`, перед обучением PPO/DQN политика предобучается
на действиях, полученных от CNN‑LSTM. Такой этап имитации особенно полезен,
когда исторических данных недостаточно: модель быстрее сходится, получая
примерные ответы от уже обученной нейросети.
...
Параметр `drawdown_penalty` задаёт штраф за просадку капитала в среде RL: из вознаграждения вычитается `коэффициент × текущая просадка`.
