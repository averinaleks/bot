...
2. Отредактируйте файл `.env`, указав в нём свои значения. При запуске основные
    ...
    - `GPT_OSS_API` — адрес сервиса [GPT OSS](https://github.com/jina-ai/gpt-oss),
      ...
      запускающий образ `ghcr.io/openai/gpt-oss:20b` на порту `8000`. Сервис
      `bot` зависит от него и получает переменную
      `GPT_OSS_API=http://gptoss:8000`. Для кеширования моделей контейнеру
