...
2. Скопируйте файл `.env.example` в `.env` и заполните свои значения. Например:
   ```bash
   cp .env.example .env
   ```
   При запуске основные скрипты вызывают `load_dotenv()` из библиотеки
   `python-dotenv`, поэтому переменные из файла подхватываются автоматически.
   При необходимости задайте торговую пару через переменную `SYMBOL`. По умолчанию используется `BTCUSDT`.
