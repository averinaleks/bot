...
2. Отредактируйте файл `.env`, указав в нём свои значения. При запуске основные
    ...
    - `GPT_OSS_API` — базовый адрес сервиса [GPT OSS](https://github.com/jina-ai/gpt-oss)
      ...
      и выполняет `python3 check_code.py` с тем же API. Пути к анализируемым
      файлам задаются переменной `CHECK_CODE_PATH` — список, разделённый
      запятыми (по умолчанию `trading_bot.py`). Все эти сервисы подключены к
      общей сети `gptoss_net`.
