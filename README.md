...
При установленном `OFFLINE_MODE=1` сервисы автоматически подставляют фиктивные значения для обязательных API-ключей. Модуль `services.offline` задаёт заглушки для `BYBIT_API_KEY`, `BYBIT_API_SECRET` и `TRADE_MANAGER_TOKEN`, а `TradeManager` подставляет значения для `TELEGRAM_BOT_TOKEN` и `TELEGRAM_CHAT_ID`. Если переменные отсутствуют, в логах появляется предупреждение об использовании заглушек, но выполнение не прерывается.
