...
3. Отредактируйте `config.json` под свои нужды. Помимо основных настроек можно
  ...
  - `portfolio_metric` задаёт метрику (`sharpe` или `sortino`), которую использует `StrategyOptimizer` при оценке портфеля.
...
предобучается на действиях, полученных от CNN‑LSTM. Такой этап имитации
