...
В каталоге `services/` есть простые эталонные сервисы, которые можно задействовать для тестирования. Основной `docker-compose.yml` по‑прежнему разворачивает полноценные модули.
