...
`pandas 2.3.2`, `numpy 2.2.6`, `httpx 0.28.1`, `psutil 7.1.0`, `scipy 1.15.3`
и `uvicorn 0.35.0`.
...
Даже если в `config.json` заданы боевые фабрики, запуск `python run_bot.py --offline` временно подменяет `service_factories` для `exchange`, `telegram_logger`, `gpt_client` и `model_builder` на офлайн-заглушки из `services.offline`. Пользовательские подстановки с пометкой `offline` (в имени модуля, пути или через атрибут `__offline_stub__ = True`) остаются без изменений, поэтому свои тестовые классы можно подключать по-прежнему.
