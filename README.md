...
- Для использования TradeManager в скриптах импортируйте его напрямую из
  ...
  обратной совместимости и будет удалена в будущих версиях.
...
- Внутренние вызовы `create_order`/`cancel_order` выполняются последовательно под
  общей блокировкой `RLock`, чтобы избежать гонок при совместном использовании
  одного экземпляра биржи несколькими потоками.
