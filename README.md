...
2. Отредактируйте файл `.env`, указав в нём свои значения. При запуске основные
    ...
    запускаются сервисы Flask. Для безопасности сервис `model_builder` по
    ...
    - `GPT_MODEL_REVISION` — хеш коммита или тег модели. Переменная обязательна
      и передаётся в параметр `revision` при загрузке модели; без неё сервис не
      запустится.
    - `GPT_OSS_API` — базовый адрес сервиса [GPT OSS](https://github.com/jina-ai/gpt-oss)
      ...
      запускающий образ `ghcr.io/openai/gpt-oss:20b` на порту `8003` (внутри
      контейнера используется порт `8000`). Сервис `bot` зависит от него и
      получает переменную `GPT_OSS_API=http://localhost:8003`. Для кеширования
      моделей контейнеру
      `gptoss` требуется переменная `TRANSFORMERS_CACHE`, указывающая на путь в
      примонтированном томе.

      Дополнительно доступен сервис `gptoss_check`, который монтирует репозиторий
      и выполняет `python3 check_code.py` с тем же API. Путь к анализируемому
      файлу задаётся переменной `CHECK_CODE_PATH` (по умолчанию `trading_bot.py`).
      Все эти сервисы подключены к общей сети `gptoss_net`.
