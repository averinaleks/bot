...
----------------------------
Для оффлайн-запуска через `run_bot.py` дополнительные секреты не нужны: переменная `CSRF_SECRET` требуется лишь при запуске FastAPI-сервера (см. `server.py`).

Запуск бота:
```bash
python run_bot.py --offline --runtime 1
```
Флаг `--offline` принудительно включает заглушки, а `--runtime` ограничивает продолжительность торгового цикла (0 — без ограничения). Скрипт подменяет фабрики сервисов `OFFLINE_SERVICE_FACTORIES`, загружает конфигурацию, инициализирует обработчик данных, построитель модели и менеджер сделок. В оффлайн-режиме допускаются предупреждения о неподставленных ключах (`BYBIT_API_KEY`, `TRADE_MANAGER_TOKEN` и др.) — они не блокируют выполнение. Логи записываются в каталог `logs/`, путь можно изменить через конфигурацию.
