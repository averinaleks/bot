...
The module keeps dependencies minimal and sticks to the standard library so it
can run in restrictive GitHub Actions contexts without extra packages.  It can
also be imported from unit tests to validate individual helper functions.
"""
...
def _send_request(api_url: str, payload: dict[str, Any], timeout: float) -> dict[str, Any]:
    ...
    request_obj = request.Request(api_url, data=data, headers=headers, method="POST")
    ...
    try:
        ...
        raise RuntimeError(
            f"Ошибка при обращении к GPT-OSS ({api_url}): {exc.reason}"
        ) from exc
