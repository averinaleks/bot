...
def _reload_sitecustomize() -> ModuleType:
    ...
    # ``sitecustomize`` is imported during interpreter start-up which means the
    # module caches environment-derived state.  Drop the cached module before
    # re-importing it so tests see the latest environment changes.
